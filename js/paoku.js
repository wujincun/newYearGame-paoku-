...
var paoku = {
    ...
    renderHouse: function (ctx) {
        ...
        var h = _this.h;
        var top,rightTop;
        _this.leftToHouseRightDistance = w * 320 / 750;//左边最小房子的右边以及右边最小房子的左边距页面两端的距离
        ...
        _this.leftStartToEndDistance = w * (650/3) / 750;                     //跑道倾斜角度为72度左右，那么移动的高是宽的3倍，大约为650/3=216；
        _this.leftBaseHouseSpeed = _this.leftStartToEndDistance / _this.totalTime;//水平方向上的速度

        _this.rightEndToHouseDistance = h * 290 / 1334; //开始渲染时最小房子在页面上的top
        _this.rightHouseDisappearDistance = h * 280 / 1334; //房子消失的位置top
        _this.rightStartToEndHouseDistance = h * 650 / 1334; //右边最大房顶到最小房顶为680
        _this.rightLeftStartToEndDistance = w * (650/3) / 750;                     //右边跑道倾斜角度为72度左右，那么移动的高是宽的3倍，大约为650/3=216；
        _this.rightBaseHouseSpeed = _this.rightLeftStartToEndDistance / _this.totalTime;//水平方向上的速度
        ...
        for (var i = 0; i < 8; i++) {
            ...
            if (i == 4) {
                ...
                _this.houseList[i].position[1] = rightTop =  _this.rightEndToHouseDistance;//位置水平竖直都变，
            }
            if (i == 5) {
                _this.houseList[i].img.src = './img/house4.png';
                _this.houseList[i].size = [w * 0.543, w * 0.421];//407*316
                _this.houseList[i].position[1] = rightTop += _this.houseList[i - 1].renderSize[1] * 1.5;//位置水平竖直都变，
            }
            if (i == 6) {
                _this.houseList[i].img.src = './img/house2.png';
                _this.houseList[i].size = [w * 0.4293, w * 0.412];//322*309
            ...
            if(i<4){
                ...
                _this.houseList[i].radio = (_this.houseList[i].position[1] - _this.rightHouseDisappearDistance) / _this.rightStartToEndHouseDistance;
                _this.houseList[i].houseSizeRadio =  _this.houseList[i].radio * (1 - _this.minHouseSizeM) + _this.minHouseSizeM;
                _this.houseList[i].renderSize = [_this.houseList[i].size[0] * _this.houseList[i].houseSizeRadio, _this.houseList[i].size[1] * _this.houseList[i].houseSizeRadio];
                _this.houseList[i].position[0] = w - (_this.leftToHouseRightDistance -  _this.houseList[i].radio* _this.rightLeftStartToEndDistance);//位置水平竖直都变，
            }
    ...
    run: function (ctx) {
        ...
        function animateRun() {
            ...
            if(_this.lastTime > 0){
                ...
                _this.leftHouseS = _this.leftBaseHouseSpeed * (curTime - _this.lastTime);
                _this.rightHouseS = _this.rightBaseHouseSpeed * (curTime - _this.lastTime)
            }
        ...
        for(var i=0;i<8;i++){
            if(i<4){
                if(_this.houseList[i].position[1] <= _this.houseDisappearDistance){
                    ...
                    _this.houseList[i].position[1] = _this.houseList[i].position[1] - _this.leftHouseS;
                }
                _this.houseList[i].radio = (_this.houseList[i].position[1] - _this.houseDisappearDistance) / _this.startToEndHouseDistance;
                _this.houseList[i].houseSizeRadio =  _this.houseList[i].radio * (1 - _this.minHouseSizeM) + _this.minHouseSizeM;
                _this.houseList[i].renderSize = [_this.houseList[i].size[0] * _this.houseList[i].houseSizeRadio, _this.houseList[i].size[1] * _this.houseList[i].houseSizeRadio];
                ...
                if(_this.houseList[i].position[1] <= _this.rightHouseDisappearDistance){
                    ...
                    _this.houseList[i].position[1] = _this.rightHouseDisappearDistance+(_this.houseList[4].renderSize[1]+_this.houseList[5].renderSize[1]+_this.houseList[6].renderSize[1]+_this.houseList[7].renderSize[1])*1.5 - maxRightHouseH;
                }else{
                    _this.rightHouseS<=_this.houseList[i].position[1] && (_this.houseList[i].position[1] = _this.houseList[i].position[1] - _this.rightHouseS);
                }
                _this.houseList[i].radio = (_this.houseList[i].position[1] - _this.rightHouseDisappearDistance) / _this.rightStartToEndHouseDistance;
                _this.houseList[i].houseSizeRadio =  _this.houseList[i].radio * (1 - _this.minHouseSizeM) + _this.minHouseSizeM;
                _this.houseList[i].renderSize = [_this.houseList[i].size[0] * _this.houseList[i].houseSizeRadio, _this.houseList[i].size[1] * _this.houseList[i].houseSizeRadio];
